---
name: retrospective
description: Post-task retrospective — extracts lessons learned into ADRs under specs/adr and reusable knowledge under specs/knowledge in the project repository
triggers:
...
```bash
git add specs/adr/ADR-{NNN}-{short-title}.md specs/knowledge/
git commit -m "docs: retrospective — ADR-{NNN} and knowledge updates"
```

Only stage paths that actually changed. If the user has not asked to commit, present the files and ask before committing.
